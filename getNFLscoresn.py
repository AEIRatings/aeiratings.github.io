...
def clean_team_name(full_name, valid_team_names):
    """
    NFL team names in data/nfl.csv already include the mascot (e.g.
    'Buffalo Bills'), so ESPN's displayName should match exactly. Fall back
    to a substring match in case of minor naming differences.
    """
    if not full_name:
        return None
    ...
    if normalized in valid_team_names:
        return normalized
    ...
    for team in valid_team_names:
        ...
        if team_no_accents == lower_no_accents or team_no_accents in lower_no_accents:
...
def fetch_upcoming_nfl_games():
    valid_team_names = load_team_names("data/nfl.csv")
    CSV_FILENAME = "data/nfl_games.csv"
    all_game_data = []
    seen_games = set()

    BASE_URL = "http://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard"

    # Pull every game from today through the end of the season (the Super
    # Bowl wraps up well before this date), rather than maintaining a
    # manually-edited schedule file.
    ...
    for i in range(0, DAYS_AHEAD):
        ...
        print(f"[{i}/{DAYS_AHEAD}] Checking NFL games for {display_date}...")
